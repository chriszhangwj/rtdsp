...
#define BUFLEN 650  /* Frame buffer length must be even for real fft */
...
void dft_func (int N, complex *X);
void idft_func (int N, complex *X);
...
void init_arrays(void)
{
    ...
    temp       = (complex *) calloc(BUFLEN, sizeof(complex)); /* temporary array */
    mag = (float *) calloc(BUFLEN, sizeof(float)); /* magnitude array*/
    result = (complex *) calloc(BUFLEN, sizeof(complex)); 
...
void wait_buffer(void)
{
	...
	for (i=0;i<BUFLEN; i++){
		temp[i] = cmplx(intermediate[i],0);
	}
	 
	dft_func (BUFLEN,temp);
	for (i=0;i<BUFLEN; i++){
		mag[i] = cabs(temp[i]);
	}
	idft_func (BUFLEN,temp);
	for (i=0;i<BUFLEN; i++){
		intermediate[i] = temp[i].r;
	}
...
void dft_func (int N, complex *X){
	...
	for (k=0;k<N; k++){
		sum = cmplx(0,0);
		for(n = 0 ; n<N; n++) {
			sum = cadd(sum,cmul (X[n], cexp(cmplx(0,-2*PI*k*n/N))));
		}
		result[k] = sum;
	}
...
void idft_func (int N, complex *X){
	...
	for (n=0;n<N; n++){
		sum = cmplx(0,0);
		for(k = 0 ; k<N; k++) {
			sum = cadd(sum,cmul (X[k], cexp(cmplx(0,2*PI*k*n/N))));
		}
		result[n] = rdiv (sum,N);
	}
